...
namespace Account_Simulator
{
    public partial class account : Form
    {
        ...
        {
            // 存款處理：解析輸入、呼叫 _bankAccount.Deposit，更新顯示
            ...
            if (decimal.TryParse(depositTextBox.Text, out decimal amount))
            {
                ...
                bool ok = _bankAccount.Deposit(amount);
                if (ok)
                {
        ...
        {
            // 提款處理：解析輸入、呼叫 _bankAccount.Withdraw，並處理餘額不足情況
            if (_bankAccount == null)
            {
                MessageBox.Show("尚未建立帳戶，請先建立帳戶。", "錯誤", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            ...
            if (decimal.TryParse(withdrawTextBox.Text, out decimal amount))
            {
                ...
                bool ok = _bankAccount.Withdraw(amount);
                if (ok)
                {
                    UpdateAccountDisplay();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (decimal.TryParse(balanceTextBox.Text, out decimal amount))
            {
                ...
                // 建立帳戶物件並存儲到 _bankAccount
                _bankAccount = new BankAccount(name, accountNumber, amount);
                UpdateAccountDisplay();
        ...
        private void UpdateAccountDisplay()
        {
            ...
            // 使用 BankAccount.ToString() 格式化顯示帳戶資訊
            balanceLabel.Text = _bankAccount.ToString();
        }
    }
}
